    ...
    private TextField getTextField() {
        ...
        textField.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if (event.getCode() == KeyCode.ESCAPE) {
                ...
                TablePosition<S, ?> pos = getTableView().getSelectionModel().getSelectedCells().getFirst();
                if (pos == null) return;
                int columnIndex = pos.getColumn();
                int rowIndex = pos.getRow();
                int nextColumnIndex = columnIndex + 1;
                if (nextColumnIndex >= getTableView().getColumns().size()) {
                    nextColumnIndex = 0;
                    rowIndex++;
                }
                if (rowIndex >= getTableView().getItems().size()) {
                    S first = getTableView().getItems().getFirst();
                    return;
                }
                getTableView().getSelectionModel().clearAndSelect(rowIndex, getTableView().getColumns().get(nextColumnIndex));
                getTableView().edit(rowIndex, getTableView().getColumns().get(nextColumnIndex));
                event.consume();
